...
open Module in
/-- Any invariant root submodule is contained in the span of the roots it contains. -/
lemma invtRootSubmodule.le_span_root {K : Type*} [Field K] [NeZero (2 : K)]
    [Module K M] [Module K N] {P : RootPairing ι K M N} [P.IsRootSystem]
    (q : P.invtRootSubmodule) :
    (q : Submodule K M) ≤
      span K (P.root '' {i | P.root i ∈ (q : Submodule K M)}) := by
  have hq_ker : ∀ ℓ, P.root ℓ ∉ (q : Submodule K _) →
      (q : Submodule K _) ≤ ker (P.coroot' ℓ) := by
    intro ℓ hℓ w hw
    rw [LinearMap.mem_ker]; by_contra h; apply hℓ
    have h_refl : P.reflection ℓ w ∈ (q : Submodule K _) :=
      P.mem_invtRootSubmodule_iff.mp q.property ℓ hw
    rw [P.reflection_apply] at h_refl
    have h_smul : (P.coroot' ℓ w) • P.root ℓ ∈ (q : Submodule K _) := by
