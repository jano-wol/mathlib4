...
This file proves that in characteristic zero, the kernel of the Killing form (the "Killing
radical") of any finite-dimensional Lie algebra is contained in the solvable radical. As a
corollary, any finite-dimensional Lie algebra with trivial radical has non-degenerate Killing
form (i.e., is a "Killing Lie algebra").

The proof uses Humphreys' lemma (from `Mathlib.Algebra.Lie.HumphreysLemmaGeneral`)
together with Engel's theorem to show the kernel of the Killing form is a solvable ideal.
-/
...
lemma killingCompl_top_le_radical :
  ...
  have ad_nil : ∀ x ∈ (SS : LieSubmodule K L L).toSubmodule,
    ...
    · intro b hb
      ...
      -- SS is an ideal of L, x ∈ SS, so ⁅x, s⁆ ∈ SS
