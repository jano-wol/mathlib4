...
noncomputable def invtSubmoduleToLieIdeal (q : Submodule K (Dual K H))
  ...
  lie_mem := by
    ...
    | mem χ x_χ hx_χ =>
      ...
      | mem α m_α hm_α => exact invtSubmoduleToLieIdeal_aux q hq χ x_χ hx_χ α m_α hm_α
      | zero =>
...
lemma eq_top_of_invtSubmodule_ne_bot (q : Submodule K (Dual K H))
  ...
  have s₅ : I ≠ ⊤ := by
    ...
    by_contra! hI
    -- If I = ⊤, then z ∈ I
    have hz_in_I : z ∈ (I : LieSubalgebra K L) := by
      rw [hI]; exact trivial
    -- But I is generated by sl₂ subalgebras for roots in q
    -- and z is in a root space for j which is not in q
    -- This leads to a contradiction via the pairing argument
    sorry
  have s₆ : I ≠ ⊥ := by
    sorry -- Follows from h₁ : q ≠ ⊥
  -- I is already a LieIdeal, so we can apply simplicity directly
