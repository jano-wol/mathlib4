...
/-- The `sl₂` subalgebra associated to a root `α`, viewed as an `H`-submodule.
      ...
      | mem χ x_χ hx_χ =>
        ...
        | mem α m_α hm_α =>
          ...
          · have h_chi_neg_alpha : χ.toLinear = -α.1.toLinear := by
            ...
            have h_bracket_in_sl2 : ⁅x_χ, m_α⁆ ∈ sl2SubalgebraOfRoot α.2.2 := by
              have hm_α_in_sl2 : m_α ∈ sl2SubalgebraOfRoot α.2.2 := by
                simp only [sl2SubalgebraOfRoot_as_H_submodule] at hm_α_original
                exact hm_α_original
          ...
          · have hx_χ_in_H : x_χ ∈ H.toLieSubmodule := by
            ...
            exact (sl2SubalgebraOfRoot_as_H_submodule α.1 α.2.2).lie_mem hm_α_original
